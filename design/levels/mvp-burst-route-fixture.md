...
- Room bounds are the complete AABB snapshot `min=(0.0,0.0,0.0)`,
  ...
  position, or published contact may exceed `y=4.0 m`. The harness-synthetic
...
then an injected-`PhysicsScene` ray query from the player-feet noise origin to the
...
pickup_interactions:
  - interaction_id: pickup_interact_gate_01
    ...
    expected_precondition:
      ...
      e20_linecast: {origin_ws: REQUIRED_RUNTIME_PLAYER_FEET_PLUS_F12_OFFSET, endpoint_ws: [1.5, 0.0, 6.0], clear: true, layer_mask: World, query_trigger_policy: QueryTriggerInteraction.Ignore}
  ...
  - interaction_id: pickup_interact_reserve_01
    ...
    expected_precondition:
      ...
      reachable: true
      player_feet_ws: REQUIRED_RUNTIME_PLAYER_FEET
      pickup_anchor_ws: [2.0, 0.0, 14.0]
      pickup_reach_radius_m: 1.6
      distance_squared_m2: REQUIRED_RUNTIME_DISTANCE_SQUARED
      distance_predicate: distance_squared(player_feet, pickup_anchor) <= pickup_reach_radius^2
      distance_within_reach: true
      e20_linecast: {origin_ws: REQUIRED_RUNTIME_PLAYER_FEET_PLUS_F12_OFFSET, endpoint_ws: [2.0, 0.0, 14.0], clear: true, layer_mask: World, query_trigger_policy: QueryTriggerInteraction.Ignore}
      patrol_corridor_clearance: {invariant: authoring_only, pickup_reach_radius_m: 1.6, patrol_agent_radius_m: 0.4, minimum_m: 2.0, result: REQUIRED_AUTHORED_CLEARANCE}
      clear_linecast: true
    expected_result: accepted
    telemetry_event: pickup-reached
...
solids:
  ...
  - id: world_initial_overlap_01
    ...
    layer: World
    is_trigger: false
    surface_role: launch_overlap_blocker
    surface_top_y_m: null
    bounds_min: [4.0, 0.0, 5.95]
    bounds_max: [4.1, 3.0, 6.05]
  ...
  - id: world_gate_closed_01
    ...
    layer: World
    is_trigger: false
    surface_role: gate_closed_obstruction
    surface_top_y_m: 2.0
...
- `patrol_pre_spend_01` — `loop_index ≥ 1`, guard state `Patrol`, no re-anchor or
  ...
  this window. Its `min_validity_s` is `2.24 s`: the phase starts at the accepted Throw edge and covers the certified lower-route flight upper bound `selected_t_land_s=1.4330082768164518 s`, `T_hearing=0.2 s`, the certified `T_fsm=0.3 s` response bound, and `0.3 s` scheduling margin through the gate decision.
...
`op: promote` and retains the existing `episode_id` and `entry_id`; it carries the
...
route_evidence:
  - route_id: gate_route_closed_01
    ...
    path_complete: false
    path_samples: [[5.5,0.0,8.0], [6.5,0.0,8.0]]
...
route_causality:
  - causality_id: gate_burst_to_exit_01
    ...
    terminal_cause: Collision_or_Timeout
    terminal_publication_state: published
    ...
    liveness_continuity:
      applicability: investigate_to_chase_only
      op: promote
      episode_id: REQUIRED_RUNTIME_EPISODE_ID
      entry_id: REQUIRED_RUNTIME_ENTRY_ID
...
`t_publish == relay.t_publish`, `terminal_publication_state=published`, and
...
valid_throw_volumes:
  - volume_id: throw_flat_starter_01
    ...
    expected:
      ...
      r_actual_m: 11.536878011794986
      theta_rad: 0.6632251157578453
      theta_rad_conversion: {source: theta_deg, formula: theta_deg * pi / 180, expected: 0.6632251157578453}
      angle_input: {theta_deg: 38.0, theta_rad: 0.6632251157578453, domain: 0 < theta_rad <= pi/4, result: PASS}
      ...
      selected_positive_root: {root_id: t_plus, t_s: 1.4640508342038154, policy: minimum_strictly_positive}
      clearance: {ceiling_margin_m: 0.518098612638639, lateral_margin_m: 0.4}
  ...
  - volume_id: throw_lower_landing_01
    ...
    expected:
      ...
      selected_positive_root: {root_id: t_plus, t_s: 1.4330082768164518, policy: minimum_strictly_positive}
      clearance: {ceiling_margin_m: 0.518098612638639, lateral_margin_m: 0.4}
...
negative_validation_cases:
  ...
  - case_id: ambiguous_collision_01
    ...
    grazing_surface_id: null
    grazing_clearance_m: null
    complete_inputs: {v0_mps: 10.0, theta_deg: 38.0, h_release_m: 1.5, h_landing_m: 0.0, delta_y_m: 1.5, launch_position_ws: [2.0, 1.5, 8.0], launch_direction_xz: [1.0, 0.0], camera_azimuth_deg: 90.0, projectile_radius_m: 0.05, pickup_id: burst_gate_pickup_01}
    configuration_result: NOT_EVALUATED
  ...
  - case_id: initial_overlap_01
    ...
    candidate_solids: []
    hit_distance_delta_m: null
    complete_inputs: {v0_mps: 10.0, theta_deg: 38.0, h_release_m: 1.5, h_landing_m: 0.0, delta_y_m: 1.5, launch_sphere_radius_m: 0.05, launch_position_ws: [4.05, 1.0, 6.0], launch_direction_xz: [1.0, 0.0], camera_azimuth_deg: 90.0, geometry_variant_id: geometry_initial_overlap_01, pickup_id: burst_gate_pickup_01}
    configuration_result: NOT_EVALUATED
...
are likewise never inferred from these authored expectations.
