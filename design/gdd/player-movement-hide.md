...
3. **Entry → the zone publishes events.** On the player's capsule entering an `Empty` spot's
   ...
   **`interior_position`** as the spot reference, (c) publishes a `HideSpot` occupied-zone
   ...
   detection). Exit publishes the corresponding empty event. At the R12 seam both are
   `HideSpotOccupancy` facts carrying `session_id`, `attempt_epoch`, `hide_spot_id`,
   `interior_position`, `entry_id` when one exists, `occupied`, `timestamp`, and
   `publisher=HideSpot`; `occupied=true/false` is the authoritative state transition.
   Both events carry `hide_spot_id` + `interior_position` verbatim.
...
### Cross-system note (FSM C1.4a synchronization — implementation evidence pending, updated 2026-08-29)

This GDD's D3/AC3/AC6/AC8/AC13/E3/E10 depend on an **occupancy-gated, exit-aborting, 4-phase-ordered capture** that the synchronized FSM C1.4a contract now specifies: the FSM's single-clock capture (C1.3/C1.4) has no `spot.state == Occupied` conjunct, no player-exit/Empty-event consumption, no **4-phase virtual-tick ordering** (`Empty` before `gate ∧ timer≥t_catch ∧ dwell` on same tick), no **same-tick exit+re-entry deferral** (fresh allocation deferred next tick), and no mid-dwell-exit-no-capture AC (and no stagger carry-forward accrual pin for simultaneous `HideSpotFront` holds). **This is a blocking dependency for *implementation*, recorded here and in Section F — a player who exits mid-dwell (or exits+re-enters same tick) is falsely captured until the FSM is amended — and the dependency remains JOINT-CONTINGENT for implementation evidence (Hide owns the synchronous flush/order + datum + witnessable harness, FSM owns the abort outcome; AC3/AC8/AC13 require the synchronized cross-system implementation).** The bounded amendment (occupancy conjunct + Empty consumption + 4-phase ordering + re-entry deferral + mid-dwell-exit AC + stagger accrual) is now cross-referenced by the FSM contract; runtime implementation and evidence remain a **standing obligation** tracked in `systems-index.md` row 1 and are NOT silently assumed. See review log. This GDD may be **APPROVED-CONTINGENT** after its own 7 pins land; implementation remains BLOCKED until FSM rev 4.2.
