...
This project pins **Unity 6 LTS (6000.x)**. Before suggesting code or architecture:

1. Read `docs/engine-reference/unity/VERSION.md` to confirm the pinned version and risk level
2. Check deprecated APIs before suggesting code — prefer Unity 6-era APIs over legacy ones
3. Check `docs/engine-reference/unity/breaking-changes.md` (if present) for relevant version transitions
4. Use WebSearch to verify any API you are uncertain about — never guess Unity API signatures
5. The new Input System (Input Actions) is standard — never suggest legacy `Input.GetKey()`
6. URP is the render pipeline — never suggest the built-in render pipeline
